...
if(!arr.length){
  index= -1;
   ...
   index= arr.indexOf( arr.reduce((a, val) => a.length > val.length ? a : val))
}
return index;
};
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.map(array=> array.filter(ele => !ele.includes('Brook')));
};
...
const sortByDay = (arr) => {
  // Solution code here...
  return daysOfWeek.map(ele => arr.filter(day => day.includes(ele)));
};
...
const characterByIndex = (arr) => {
  // Solution code here...
  return arr.map((ele,i) => ele.substring(i,i+1));
};
